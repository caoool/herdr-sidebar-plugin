...
/** Where a reading came from. All are pull-free except `api`. */
...
export type QuotaSnapshot = {
  ...
  /**
   * When the AGENT observed these numbers, taken from its own timestamp — not when we read
   * the file. A watched file can be arbitrarily old if that agent has been idle.
   */
  observedAt: number
  source: QuotaSource
  error?: string
